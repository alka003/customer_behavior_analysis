...
USE shopping_db;
CREATE USER 'eda_user'@'localhost' IDENTIFIED BY 'eda_password';
GRANT ALL PRIVILEGES ON shopping_db.* TO 'eda_user'@'localhost';
FLUSH PRIVILEGES;
SHOW TABLES;
...
select gender,
sum(purchase_amount) as revenue from customer_data 
group by gender;
...
round(sum(purchase_amount),2) as total_revenue from customer_data
